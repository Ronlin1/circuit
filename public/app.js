...
async function api(path,options={}){
  ...
  if(!response.ok||!payload.ok) throw new Error(payload.error?.message||payload.error?.code||'CIRCUIT request failed');
  return payload.data;
}
...
function toast(title,message){const node=document.createElement('div');node.className='toast';node.innerHTML=`<strong>${escapeHtml(title)}</strong><br>${escapeHtml(message)}`;$('#toast-region').append(node);setTimeout(()=>node.remove(),4200)}
