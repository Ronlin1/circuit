...
async function withServer(fn){
  ...
  try{await fn(`http://127.0.0.1:${port}`,()=>marketCalls)}finally{await new Promise(resolve=>server.close(resolve));services.close()}
}
...
test('live page evaluation fetches fresh market evidence server-side and never executes Binance writes',async()=>withServer(async(base,getCalls)=>{
...
test('Binance public provider implementation is read-only and uses the Spot 24h ticker endpoint',async()=>{
